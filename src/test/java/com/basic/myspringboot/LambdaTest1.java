...
public class LambdaTest1 {
    ...
    public void transformUsingStream(){
        ...
        //Customer의 이름 목록(Name List) 추출 List<String>
        //List<MyCustomer> => List<String>
        customers.stream() //List<MyCustomer> => Stream<MyCustomer>
                //map(Funtion) Funtion의 추상메서드 R apply(T t)
        ...
        //id가 103보다 큰 Customer의 이름 추출
        customers.stream() //List<MyCustomer> => Stream<MyCustomer>
                //map(Funtion) Funtion의 추상메서드 R apply(T t)
                .filter(customer -> customer.getId() >= 103)  //Stream<MyCustomer> => Stream<String>
